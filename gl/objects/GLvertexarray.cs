[StructLayout(LayoutKind.Sequential)]
public struct GLvertexarray : IEquatable<GLvertexarray>, IEquatable<GLuint> {
    public static readonly GLvertexarray Null = default;

    readonly GLuint m_handle;

    internal GLvertexarray(GLuint handle) {
        m_handle = handle;
    }

    #region IEquatable<GLvertexarray>, IEquatable<GLuint>

    public override bool Equals(object obj) {
        if (obj is GLvertexarray objHandle) {
            return Equals(objHandle);
        }
        if (obj is GLuint handle) {
            return Equals(handle);
        }
        return base.Equals(obj);
    }
    public bool Equals(GLvertexarray other) => m_handle.Equals(other.m_handle);
    public bool Equals(GLuint other) => m_handle.Equals(other);
    public override int GetHashCode() => m_handle.GetHashCode();
    public override string ToString() => m_handle.ToString();

    public static bool operator ==(GLvertexarray a, GLvertexarray b) => a.m_handle.Equals(b.m_handle);
    public static bool operator ==(GLvertexarray a, GLuint b) => a.m_handle.Equals(b);
    public static bool operator ==(GLuint a, GLvertexarray b) => a.Equals(b.m_handle);

    public static bool operator !=(GLvertexarray a, GLvertexarray b) => !a.m_handle.Equals(b.m_handle);
    public static bool operator !=(GLvertexarray a, GLuint b) => !a.m_handle.Equals(b);
    public static bool operator !=(GLuint a, GLvertexarray b) => !a.Equals(b.m_handle);

    public static explicit operator GLuint(GLvertexarray obj) => obj.m_handle;
    public static explicit operator GLvertexarray(GLuint handle) => new(handle);

    #endregion
}